...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lenVector1 = Math.sqrt(x1 * x1 + y1 * y1);
  const lenVector2 = Math.sqrt(x2 * x2 + y2 * y2);

  if (lenVector1 === 0 || lenVector2 === 0) {
    return null;
  }
  const angleValue = vector / (lenVector1 * lenVector2);

  const angle = Math.acos(angleValue);

  if (angle.toFixed(2) === 3.14) return 'π';
  if (angle.toFixed(2) === 1.57) return 'π/2';
  if (angle.toFixed(0) === 0) return 0;
